    ...
    def update(self, scene_info):
        ...
        if not self.ball_served:
            ...
            if self.side == "1P":
                ...
                if scene_info["platform_1P"][0]+20  > (pred-5)and scene_info["platform_1P"][0]+20 < (pred+5):
                    if scene_info["platform_1P"][1]+30-scene_info["ball_speed"][1] > scene_info["ball"][1] : #slice
                        return "NONE"
                    else :
                        return "NONE" # NONE
                elif scene_info["platform_1P"][0]+20 <= (pred-5) : return "MOVE_RIGHT" # goes right
                ...
                X = np.array(X).reshape((1,-1))
                pred = self.model.predict(X)
                if scene_info["platform_2P"][0]+20  > (pred-5)and scene_info["platform_2P"][0]+20 < (pred+5):
                    if scene_info["platform_2P"][1]+30-scene_info["ball_speed"][1] > scene_info["ball"][1] : #slice
                        return "NONE"
                    else :
                        return "NONE" # NONE
                elif scene_info["platform_2P"][0]+20 <= (pred-5) : return "MOVE_RIGHT" # goes right
                else : return "MOVE_LEFT" # goes left
